...
//filter starts
  ...
  const message = document.getElementById('no-stock-message');
if (visibleCount === 0) {
  ...
  applyButton.addEventListener('click', function () {
    ...
    const selectedSkin = document.querySelectorAll('input[name="Normal"]:checked');
    const selectedMakeup = document.querySelectorAll('input[name="makeup"]:checked');
    const selectedColor = document.querySelectorAll('input[name="color"]:checked');

    // Fake logic: simulate no product found when both Skin and Makeup are selected
    if (selectedSkin.length > 0 && selectedMakeup.length > 0) {
      // You can modify this to include product filtering logic
      // Show the message
      noStockMessage.style.display = 'block';
    } else {
      // Hide message when filters are not conflicting
